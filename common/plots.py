...
def plot_momentum(minute_df: pd.DataFrame,
    ...
    ax.set_xlabel("Minuto", fontsize=6); ax.set_ylabel("Peso por minuto", fontsize=6)
...
def plot_smoothed(minute_df: pd.DataFrame,
    ...
    ax.set_xlabel("Minuto", fontsize=6); ax.set_ylabel("EWMA", fontsize=6)
    ax.tick_params(axis="both", labelsize=6)
...
# ---------------- Graph 3: Top players (horizontal bars) ----------------
def plot_top_players(df_goles: pd.DataFrame,
    ...
    ax.invert_yaxis()
    ax.set_xlabel("Participación ofensiva (intentos + 2×goles)", fontsize=6)
    ax.tick_params(axis="both", labelsize=6)
...
# ---------------- Graph 4: Cumulative attack rate ----------------
def plot_cumulative(df_goles: pd.DataFrame,
    ...
    ax.set_xlabel("Minuto", fontsize=6); ax.set_ylabel("Participación acumulada", fontsize=6)
    ax.tick_params(axis="both", labelsize=6)
    
    return ax


# --- NEW: Events by team (simple two-bar chart) ---
...
# --- NEW: Event distribution by team (grouped bars) ---
